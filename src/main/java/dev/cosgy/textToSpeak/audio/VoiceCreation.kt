...
class VoiceCreation( // 各種設定の値を保持するためのフィールド
    ...
    private val dictionaryDir: String? = bot.config.dictionaryDir
    private val voiceDir: String? = bot.config.voiceDir
    private val winJTalkDir: String? = bot.config.winExecutableDir
    private val maxMessageLength: Int = bot.config.maxMessageLength
    ...
    private fun sanitizeMessage(message: String): String {
        ...
        while (sentences.next() != BreakIterator.DONE) {
            ...
            if (maxMessageLength > 0 && sentence.length + builder.length > maxMessageLength) {
                builder.append("以下略")
                break
            }
    ...
    private val characterCode: String
        ...
        get() {
            ...
            return if (bot.config.utf8Forced) "UTF-8" else "Shift-JIS"
        }
    ...
    private fun getVoiceFilePath(voice: String?): String? {
        return voiceDir?.let { Paths.get(it, "$voice.htsvoice").toString() }
    }
    ...
    val voices: List<String>
        get() {
            val voiceDir = File(requireNotNull(voiceDir) { "voiceDirectory is null" })
